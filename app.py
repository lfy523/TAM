...
def sam_refine(video_state, point_prompt, click_state, interactive_state, evt:gr.SelectData):
    ...
    mask, logit, painted_image = Get_mask(video_state["origin_images"][video_state["select_frame_number"]])
    # print('mask', type(mask), mask.shape)
    
    # mask1, logit, painted_image = model.first_frame_click( 
    #                                                   image=video_state["origin_images"][video_state["select_frame_number"]], 
    #                                                   points=np.array(prompt["input_point"]),
    #                                                   labels=np.array(prompt["input_label"]),
    #                                                   multimask=prompt["multimask_output"],
    #                                                   )
    Image.fromarray(mask).save('mask1.png')
    # Image.fromarray(mask1).save('mask.png')
    # print(mask.shape,mask1.shape, np.max(mask), np.max(mask1),mask, mask1)
...
with gr.Blocks() as iface:
    ...
    gr.Examples(
        examples=[os.path.join(os.path.dirname(__file__), "./test_sample/", test_sample) for test_sample in ["test-sample8.mp4","test-sample1.mp4", \
